import argparse
import os
from src.common.defaults import CLOUD_CONFIG_DIR, VM_DISK_DIR, IMAGE_CONFIG_DIR
...
def generate_image_configuration(user_data_path, metadata_path, vendordata_path, output_path):
    ...
    if not exists(metadata_path):
        print(
            "Failed to find a cloud-init meta-data file at: {}".format(
                metadata_path
            )
        )
        return False
    ...
    localds_command = ["cloud-localds", output_path, user_data_path, metadata_path, "-V", vendordata_path]
...
if __name__ == "__main__":
    ...
    generated = generate_image_configuration(image_path, user_data_path, seed_image_path)
    if not generated:
